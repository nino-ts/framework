...
export class MorphToMany<
	TRelated extends Model = Model,
	TParent extends Model = Model,
> extends Relation<TRelated, TParent> {
	...
	addConstraints(): void {
		...
		if (parentKeyValue) {
			this.query
				.join(
					this.table,
					`${this.query.fromTable}.${this.relatedKey}`,
					"=",
					`${this.table}.${this.relatedId}`,
				)
				.where(
					`${this.table}.${this.morphId}`,
					"=",
					parentKeyValue as WhereClauseValue,
				)
	...
	addEagerConstraints(models: Model[]): QueryBuilder<TRelated> {
		const keys = models
			...
			.filter(
				(key): key is NonNullable<WhereClauseValue> =>
	...
	initRelation(models: Model[], relation: string): Model[] {
		for (const model of models) {
			model.setRelation(
				relation,
				new Collection<TRelated>() as unknown as Collection<Model>,
			);
		}
		return models;
	}
	...
	async attach(
		...
		const parentKeyValue = this.parent.getAttribute(
			this.parentKey,
		) as WhereClauseValue;
		const parentType = this.parent.getTable();
		...
		const bindings = Object.values(pivotData).filter(
			(v): v is NonNullable<unknown> => v != null,
		);
	...
	async sync(
		...
		const idArray = Array.isArray(ids)
			? ids
			: Object.keys(ids).map((k) => k as unknown as WhereClauseValue);
		...
		const currentIds = current
			.all()
			.map(
				(item: TRelated): PrimaryKey =>
					item.getAttribute(this.relatedKey) as PrimaryKey,
			);

		// Detach missing
		if (detaching) {
			const toDetach = currentIds.filter(
				(id: PrimaryKey) => !idArray.includes(id as WhereClauseValue),
			);
