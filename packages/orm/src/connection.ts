...
export class Connection implements Connector {
	...
	constructor(config: ConnectionConfig) {
		...
		const driverName =
			config.driver === "postgresql" ? "postgres" : config.driver;
	...
	private serializeBindings(
		...
		return bindings.map(
			(value: WhereClauseValue): string | number | boolean | null => {
	...
	async run(
		...
		switch (this.driver) {
			...
			case "mysql": {
				...
				return {
					changes:
						(result as unknown as { count?: number; affectedRows?: number })
							.count ??
			...
			default: {
				const exhaustiveCheck: never = this.driver;
				throw new Error(
					`Statement execution not supported for driver [${exhaustiveCheck}]`,
				);
			}
		}
	}
		...
		switch (this.driver) {
			...
			case "mysql": {
				...
				return await db.begin(async (tx) => {
					...
					const txConnection = new TransactionConnection(
						tx,
						this.driver,
						this.config,
					);
			...
			default: {
				const exhaustiveCheck: never = this.driver;
				throw new Error(
					`Transactions not supported for driver [${exhaustiveCheck}]`,
				);
			}
		}
	}
