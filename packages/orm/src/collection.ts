...
export class Collection<T> implements Iterable<T> {
	...
	filter(callback: (item: T, index: number) => boolean): Collection<T> {
		return new Collection<T>(
			this.items.filter((item, index) => callback(item, index)),
		);
	}
				...
				if (relationType === "HasMany" || relationType === "HasOne") {
					...
					if (relationType === "HasMany") {
						...
						for (const model of models) {
							const related = relatedModels.filter(
								(r: ModelInstance): boolean => {
									const fkValue = (r as Record<string, unknown>)[
										relation.foreignKey
									];
					...
					for (const model of models) {
						...
						const related = relatedModels
							.all()
							.find((r: ModelInstance): boolean => {
								const ownerValue = (r as Record<string, unknown>)[
									relation.ownerKey
								];
