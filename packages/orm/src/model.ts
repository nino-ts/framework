 ...
 */
export interface ModelConstructorWithMetadata<T extends object = object>
	extends ModelConstructor<T> {
	/**
	...
	implements ModelInstance
{
	...
	constructor(attributes: Partial<TAttributes> = {}) {
		...
		this._modelClass = this
			.constructor as ModelConstructorWithMetadata<TAttributes>;
		...
		return new Proxy(this, {
			get(target, prop, receiver): unknown {
				...
				const modelConstructor =
					target.constructor as ModelConstructorWithMetadata;
				...
				if (
					typeof prop === "string" &&
					Object.hasOwn(target.attributes, prop)
				) {
	...
	setAttribute(key: string, value: unknown): void {
		this.attributes[key as keyof TAttributes] =
	...
	newQuery(): QueryBuilder<Model<TAttributes>> {
		...
		builder.setModel(
			this._modelClass as unknown as ModelConstructor<Model<TAttributes>>,
		);
		return builder;
	}
		...
		if (this.exists) {
			...
			if (id) {
				await query
					...
					.update(this.attributes as unknown as MutationValues);
			}
			this.syncOriginal();
			return true;
		} else {
			...
			const result = await query.insert(
				this.attributes as unknown as MutationValues,
			);
			this.exists = true;
	...
	static async findOrFail<T extends Model>(
		...
		if (!result) {
			...
			throw new ModelNotFoundException(
				`No query results for model [${modelName}] ${id}`,
			);
		}
		return result;
	}
	...
	static async raw<T = Record<string, unknown>>(
		...
		const sql = strings.reduce(
			(result, str, i) => result + str + (i < values.length ? "?" : ""),
			"",
		);
	...
	belongsToMany<TRelated extends Model<Record<string, unknown>>>(
		...
		relatedKey =
			relatedKey || (instance.constructor as typeof Model).primaryKey;
