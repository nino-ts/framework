...
export class QueryBuilder<TModel extends ModelInstance = ModelInstance> {
	...
	protected async eagerLoadRelations<T extends ModelInstance>(
		...
		for (const relationName of this.eagerRelations) {
			...
			const relationMethod = (firstModel as Record<string, unknown>)[
				relationName
			];
			...
			if (relationType === "HasMany") {
				...
				const parentIds = models
					.map(
						(m: T): WhereClauseValue =>
							(m as Record<string, unknown>).id as WhereClauseValue,
					)
					.filter(
						(id): id is NonNullable<WhereClauseValue> =>
				...
				for (const model of models) {
					...
					if (
						"setRelation" in model &&
						typeof model.setRelation === "function"
					) {
				...
				const foreignKeyValues = models
					...
					.filter(
						(val): val is NonNullable<WhereClauseValue> =>
				...
				for (const model of models) {
					const fkValue = (model as Record<string, unknown>)[
						relation.foreignKey
					];
					const parent = parentModels
						.all()
						.find((p: ModelInstance): boolean => {
							const ownerValue = (p as Record<string, unknown>)[
								relation.ownerKey
							];
	...
	async insertMany(
		...
		const bindings = values.flatMap((v) =>
			columns.map((col) => v[col] ?? null),
		);
	...
	async min(column: string): Promise<number | null> {
		...
		const sql = this.toSql();
		const result = await this.connection.query<{ min: number }>(
			sql,
			this.bindings,
		);
	...
	async max(column: string): Promise<number | null> {
		...
		const sql = this.toSql();
		const result = await this.connection.query<{ max: number }>(
			sql,
			this.bindings,
		);
	...
	async sum(column: string): Promise<number | null> {
		...
		const sql = this.toSql();
		const result = await this.connection.query<{ sum: number }>(
			sql,
			this.bindings,
		);
	...
	async avg(column: string): Promise<number | null> {
		...
		const sql = this.toSql();
		const result = await this.connection.query<{ avg: number }>(
			sql,
			this.bindings,
		);
	 ...
	 */
	async pluck<T extends ModelInstance = TModel>(
		column: string,
