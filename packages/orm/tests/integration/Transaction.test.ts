...
describe("Transaction", () => {
	...
	beforeEach(async () => {
		...
		await conn.run(
			"CREATE TABLE accounts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, balance INTEGER)",
		);
		await conn.run("INSERT INTO accounts (name, balance) VALUES (?, ?)", [
			"Alice",
			1000,
		]);
		await conn.run("INSERT INTO accounts (name, balance) VALUES (?, ?)", [
	...
	test("transaction should commit changes", async () => {
		...
		const alice = await conn.query<{ balance: number }>(
			"SELECT balance FROM accounts WHERE name = ?",
			["Alice"],
		);
		const bob = await conn.query<{ balance: number }>(
			"SELECT balance FROM accounts WHERE name = ?",
			["Bob"],
		);
	...
	test("transaction() helper function should work", async () => {
		...
		const result = await transaction(conn, async (tx) => {
			...
			const bob = await tx.query<{ balance: number }>(
				"SELECT balance FROM accounts WHERE name = ?",
				["Bob"],
			);
	...
	test("query() should throw on non-active transaction", async () => {
		...
		await expect(tx.query("SELECT 1")).rejects.toThrow(
			"Transaction is committed, cannot execute queries",
		);
	});
	...
	test("run() should throw on non-active transaction", async () => {
		...
		await expect(tx.run("SELECT 1")).rejects.toThrow(
			"Transaction is rolled_back, cannot execute queries",
		);
	});
