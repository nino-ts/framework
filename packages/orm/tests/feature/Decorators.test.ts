...
describe("Decorators", () => {
	...
	test("@Column decorator should map property to database column", () => {
		...
		expect(
			(User as unknown as { __columnMapping: Record<string, string> })
				.__columnMapping,
		).toBeDefined();
		expect(
			(User as unknown as { __columnMapping: Record<string, string> })
				.__columnMapping.name,
		).toBe("full_name");
	});
	...
	test("@Column decorator should work with multiple properties", () => {
		...
		const mapping = (
			TestUser as unknown as { __columnMapping: Record<string, string> }
		).__columnMapping;
	...
	test("@Table decorator should work without explicit context", () => {
		...
		TableDecorator(
			SimpleUser,
			undefined as unknown as ClassFieldDecoratorContext,
		);

		expect((SimpleUser as unknown as { table: string }).table).toBe(
			"simple_table",
		);
	});
