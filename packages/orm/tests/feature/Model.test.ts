...
describe("Model", () => {
	...
	test("save() should insert record", async () => {
		...
		const stored = await db
			.connection()
			.query("SELECT * FROM users WHERE id = ?", [user.id as WhereClauseValue]);
	...
	test("create() should persist and return instance", async () => {
		...
		const stored = await db
			...
			.query("SELECT * FROM users WHERE id = ?", [
				created.id as WhereClauseValue,
			]);
	...
	test("firstOrCreate() should return existing model", async () => {
		...
		const result = await User.firstOrCreate(
			{ email: "existing@example.com" },
			{ name: "Ignored" },
		);
	...
	test("save() should respect non-incrementing string keys", async () => {
		...
		await db
			.connection()
			.run("CREATE TABLE api_keys (id TEXT PRIMARY KEY, name TEXT)");
