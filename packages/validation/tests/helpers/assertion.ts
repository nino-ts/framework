...
export function assertPass<T>(
	...
	if (!result.success) {
		...
		throw new Error(
			`Expected validation to pass, but got issues: ${JSON.stringify(issues)}`,
		);
	}
...
export function assertFail(
	result:
		| StandardSchemaSuccessResult<unknown>
		| StandardSchemaFailureResult<unknown>,
...
export function assertTypeSafety<T extends StandardSchemaV1>(
	...
	if (!validResult.success) {
		throw new Error(
			`Expected valid value to pass, but got: ${JSON.stringify(validResult.issues)}`,
		);
	}
