...
describe("FASE 11: i18n System", () => {
	describe("Translator Class", () => {
		...
		test("should change locale dynamically", () => {
			...
			expect(message).toBe(
				"The user_email field must be a valid email address.",
			);
		});
		...
		test("should translateMany messages", () => {
			const translator = new Translator("pt-BR");
			const messages = translator.translateMany(
				["required", "email", "min_length"],
				{
			...
			expect(messages.min_length).toBe(
				"O campo email deve ter pelo menos 3 caracteres.",
			);
		});
	...
	describe("Global Functions", () => {
		...
		test("should change global locale", () => {
			...
			expect(message).toBe(
				"El campo correo debe ser una dirección de email válida.",
			);
		});
	});
	...
	describe("Placeholder Replacement", () => {
		...
		test("should handle array values", () => {
			...
			expect(message).toBe(
				"O campo email é obrigatório quando username está presente.",
			);
		});
	});
});
