...
> = {
	[K in keyof T]: T[K] extends StandardSchemaV1<infer Input, unknown>
		? Input
		: never;
};
...
export type InferObjectOutput<
	T extends Record<string, StandardSchemaV1<unknown, unknown>>,
> = {
	[K in keyof T]: T[K] extends StandardSchemaV1<unknown, infer Output>
		? Output
		: never;
};
...
class ObjectTypeRule
	implements StandardSchemaRule<unknown, Record<string, unknown>>
{
...
export class ObjectSchema<
	T extends Record<string, StandardSchemaV1<unknown, unknown>> = Record<
		string,
...
> extends BaseSchema<Record<string, unknown>, InferObjectOutput<T>> {
	...
	public constructor(shape: T) {
		...
		this.addRule(
			new ObjectTypeRule() as unknown as StandardSchemaRule<
	...
	public minKeys(count: number): this {
		return this.addRule(
			new MinKeysRule(count) as unknown as StandardSchemaRule<
				Record<string, unknown>,
				InferObjectOutput<T>
			>,
		);
	...
	public maxKeys(count: number): this {
		return this.addRule(
			new MaxKeysRule(count) as unknown as StandardSchemaRule<
				Record<string, unknown>,
				InferObjectOutput<T>
			>,
		);
