...
describe("FilesystemManager", () => {
	...
	test("throws exception when resolving unsupported drivers", () => {
		const manager = new FilesystemManager(config);
		expect(() => manager.disk("missing")).toThrow(
			"Filesystem driver [unknown] is not supported.",
		);
		expect(() => manager.disk("not_configured")).toThrow(
			"Filesystem disk [not_configured] is not configured.",
		);
	});
