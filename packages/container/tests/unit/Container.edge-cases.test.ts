...
describe("Container - Edge Cases", () => {
	...
	describe("Factory Error Handling", () => {
		...
		test("should propagate singleton factory errors", () => {
			...
			expect(() => container.make("failing-singleton")).toThrow(
				"Singleton factory failed",
			);
		});
		...
		test("should not cache instance when singleton factory fails", () => {
			...
			expect(() => container.make("conditional-fail")).toThrow(
				"First attempt failed",
			);
			expect(
				container.make<{ attempts: number }>("conditional-fail").attempts,
			).toBe(2);
		});
	});
	...
	describe("Circular Dependencies", () => {
		test("should detect direct circular dependency", () => {
			...
			expect(() => container.make("service-a")).toThrow(
	...
	describe("Instance Registration", () => {
		...
		test("should override instance with new instance", () => {
			...
			expect(container.make<typeof firstInstance>("service")).toStrictEqual(
				firstInstance,
			);

			container.instance("service", secondInstance);
			expect(container.make<typeof secondInstance>("service")).toStrictEqual(
				secondInstance,
			);
		});
	});
