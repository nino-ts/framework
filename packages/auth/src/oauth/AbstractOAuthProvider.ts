...
export abstract class AbstractOAuthProvider implements OAuthProvider {
	...
	async user(request: Request): Promise<OAuthUser> {
		...
		oauthUser.token = response.access_token;
		oauthUser.refreshToken = response.refresh_token || null;
		oauthUser.expiresIn = response.expires_in || null;
	...
	protected async getAccessTokenResponse(
		code: string,
