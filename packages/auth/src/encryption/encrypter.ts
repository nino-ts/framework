...
export class WebEncrypter implements Encrypter {
	...
	async encrypt(value: unknown, serialize = true): Promise<string> {
		...
		const iv = (
			crypto.getRandomValues
				...
				: (crypto.randomBytes as any)(ivLength)
		) as Uint8Array;
		...
		try {
			...
			const encryptedBuffer = await (crypto.subtle as any).encrypt(
				algorithm,
				cryptoKey as any,
				data,
			);
	...
	async decrypt(payload: string, unserialize = true): Promise<unknown> {
		...
		if (
			!crypto.timingSafeEqual(Buffer.from(expectedMac), Buffer.from(parsed.mac))
		) {
	...
	private async hash(iv: string, value: string): Promise<string> {
		const hmacKey = await (crypto.subtle as any).importKey(
			"raw",
			this.key.buffer ?? this.key,
			{ hash: "SHA-256", name: "HMAC" } as any,
			false,
			["sign"],
		);
	...
	private async importAesKey(usage: "encrypt" | "decrypt"): Promise<CryptoKey> {
		...
		return (crypto.subtle as any).importKey(
			"raw",
			this.key.buffer ?? this.key,
			{ name: isGCM ? "AES-GCM" : "AES-CBC" } as any,
			false,
			[usage],
		);
	}
