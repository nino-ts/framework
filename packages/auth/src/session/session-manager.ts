...
} from "@/session/drivers/database-driver.ts";
...
export class SessionManager {
	...
	driver(name?: string): SessionDriver {
		...
		const existingDriver = this.drivers.get(driverName);
		if (existingDriver) {
			return existingDriver;
		}

		const newDriver = this.createDriver(driverName);
		this.drivers.set(driverName, newDriver);
		return newDriver;
	}
	...
	protected createFileDriver(): SessionDriver {
		return new FileSessionDriver(
			this.config.files || "./storage/framework/sessions",
		);
	}
	...
	build(driver: SessionDriver, id?: string, token?: string): Session {
		return new Session(
			driver,
			this.config.cookie || "ninots_session",
			id,
			token,
		);
	}
