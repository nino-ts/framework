...
export class JwtDecoder {
		...
		try {
			...
			if (header.alg === "RS256") {
				if (publicKey.kty !== "RSA") {
					throw new JwtError(
						`Algorithm mismatch: token uses RS256 but key type is ${publicKey.kty}`,
					);
				}
				...
				if (publicKey.kty !== "EC") {
					throw new JwtError(
						`Algorithm mismatch: token uses ES256 but key type is ${publicKey.kty}`,
					);
				}
	...
	private base64UrlDecode(str: string): string {
		try {
			...
			throw new JwtError(
				`Failed to decode base64url: ${(error as Error).message}`,
			);
		}
	}
}
