...
describe("GitHubProvider", () => {
	...
	describe("initialization", () => {
		it("should store config", () => {
			expect((provider as unknown as Record<string, unknown>).config).toEqual(
				config,
			);
		});

		it("should initialize with provided scopes", () => {
			expect((provider as unknown as Record<string, unknown>).scopes).toEqual([
				"read:user",
			]);
		});
	...
	describe("getTokenUrl()", () => {
		it("should return GitHub token endpoint", () => {
			expect(provider.getTokenUrl()).toBe(
	...
	describe("mapUserToObject()", () => {
		it("should map GitHub user data to OAuthUser", () => {
			const githubUser = {
			...
			const oauthUser = provider.mapUserToObject(githubUser);

			expect(oauthUser).toBeInstanceOf(OAuthUser);
			expect(oauthUser.getId()).toBe("12345");
			expect(oauthUser.getName()).toBe("The Octocat");
			expect(oauthUser.getEmail()).toBe("octocat@github.com");
			expect(oauthUser.getAvatar()).toBe(
		...
		it("should fallback to login when name is not provided", () => {
			...
			expect(oauthUser.getName()).toBe("github_user");
		});
		...
		it("should store entire GitHub object as raw data", () => {
			...
			expect(oauthUser.getRaw()).toEqual(githubUser);
		});
		...
		it("should set empty token (populated by parent)", () => {
			...
			expect(oauthUser.getToken()).toBe("");
		});
		...
		it("should set null for refreshToken and expiresIn", () => {
			...
			expect(oauthUser.getRefreshToken()).toBeNull();
			expect(oauthUser.getExpiresIn()).toBeNull();
		});
	});

	describe("with() method (from AbstractOAuthProvider)", () => {
