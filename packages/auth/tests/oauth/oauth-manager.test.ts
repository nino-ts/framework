...
import {
	...
	type ProviderConfig,
} from "@/oauth/AbstractOAuthProvider.ts";
...
describe("OAuthManager", () => {
	...
	describe("driver() method", () => {
		...
		it("should throw when config for driver not found", () => {
			try {
				...
				expect((error as Error).message).toContain("Config for oauth driver");
		...
		it("should create separate instances for different drivers (without extend)", () => {
			...
			let github: OAuthProvider | null = null;
		...
		it("should pass correct config to driver", () => {
			...
			expect((driver as unknown as Record<string, unknown>).config).toEqual(
				config.github,
			);
		});
	});
	...
	describe("extend() method", () => {
		...
		it("should return this for chaining", () => {
			const result = manager.extend(
				"mock",
				() => new MockProvider(config.mock),
			);
	...
	describe("driver caching", () => {
		it("should cache drivers per instance", () => {
			const manager1 = new OAuthManager({ github: config.github });
			const manager2 = new OAuthManager({ github: config.github });
	...
	describe("configuration validation", () => {
		it("should handle empty config gracefully", () => {
			const emptyManager = new OAuthManager({});
		...
		it("should work with multiple providers in config", () => {
			...
			const multiManager = new OAuthManager(multiConfig);
	...
	describe("OAuthProvider contract compliance", () => {
		it("should return providers implementing OAuthProvider interface", () => {
			const driver = manager.driver("github");
