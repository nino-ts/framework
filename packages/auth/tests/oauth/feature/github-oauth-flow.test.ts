...
function createMockFetch() {
	return async (
		...
		const url =
			typeof urlOrRequest === "string" ? urlOrRequest : urlOrRequest.toString();
		...
		if (url.includes("github.com/login/oauth/access_token")) {
			...
			if (body.code_verifier) {
				...
				if (!body.code_verifier.startsWith("mock_")) {
					// Accept any verifier for now (AbstractOAuthProvider generates real base64url)
				}
			}
...
describe("GitHub OAuth Flow — Integration Tests", () => {
	...
	describe("Authorization redirect", () => {
		...
		it("should include PKCE code_challenge when usePkce=true", async () => {
			const { provider, extractCodeChallengeFromUrl } =
	...
	describe("Token exchange and user fetch", () => {
		it("should exchange authorization code for access token", async () => {
			const { provider, createCallbackRequest, extractStateFromUrl } =
				createGitHubOAuthFixture();
			...
			const oauthUser = await provider.user(callbackRequest);

			// Verify token was exchanged and user fetched
			expect(oauthUser.token).toBe("gho_mock_access_token_abcd1234");
		});
		...
		it("should fetch user profile with access token", async () => {
			...
			expect(oauthUser.id).toBe("12345678");
			expect(oauthUser.name).toBe("The Octocat");
			expect(oauthUser.email).toBe("octocat@github.com");
			expect(oauthUser.avatar).toBe(
				"https://avatars.githubusercontent.com/u/12345678",
			);
		});

		it("should populate OAuthUser with token and refresh token", async () => {
			const { provider, createCallbackRequest, extractStateFromUrl } =
				createGitHubOAuthFixture();
			...
			expect(oauthUser.token).toBe("gho_mock_access_token_abcd1234");
			expect(oauthUser.refreshToken).toBe("ghr_mock_refresh_token_xyz789");
			expect(oauthUser.expiresIn).toBe(28800);
		});
		...
		it("should send PKCE code_verifier in token exchange request", async () => {
			...
			let tokenRequestBody: Record<string, string> | null = null;
			...
			const callbackRequest = new Request(
				`http://localhost:3000/auth/github/callback?code=mock_code&state=${state}`,
			);
	...
	describe("State validation (CSRF protection)", () => {
		it("should validate state parameter matches original", async () => {
			...
			const validRequest = new Request(
				`${redirectResponse.headers.get("Location")}?code=abc&state=${state}`,
			);
		...
		it("should throw error when state parameter is invalid", async () => {
			...
			const invalidRequest = createCallbackRequest(
				"mock_code",
				"invalid_state_xyz",
			);

			await expect(provider.user(invalidRequest)).rejects.toThrow(
		...
		it("should clear state after successful user() call", async () => {
			...
			const callbackRequest2 = createCallbackRequest(
				"code_second",
				"any_state",
			);
	...
	describe("Multiple OAuth flows", () => {
		it("should support multiple independent OAuth flows", async () => {
			...
			const { provider: provider2, createCallbackRequest: createCallback2 } =
				createGitHubOAuthFixture();
