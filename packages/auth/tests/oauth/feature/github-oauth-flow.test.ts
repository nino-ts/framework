...
describe('GitHub OAuth Flow — Integration Tests', () => {
  ...
  describe('Token exchange and user fetch', () => {
    ...
    it('should fetch user profile with access token', async () => {
      ...
      expect(oauthUser.avatar).toBe('https://avatars.githubusercontent.com/u/12345678');
    });
    ...
    it('should send PKCE code_verifier in token exchange request', async () => {
      ...
      const mockFetchWithCapture = async (
        ...
        if (url.includes('github.com/login/oauth/access_token')) {
          tokenRequestBody = JSON.parse((init?.body as string) || '{}') as Record<string, string>;
      ...
      const callbackRequest = new Request(`http://localhost:3000/auth/github/callback?code=mock_code&state=${state}`);
