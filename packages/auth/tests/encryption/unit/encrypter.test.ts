...
describe("WebEncrypter", () => {
	...
	it("throws an error if the key length is invalid", () => {
		expect(() => new WebEncrypter(key16, "AES-256-CBC")).toThrow(
			"The key length must be 32 bytes for AES-256-CBC.",
		);

		expect(() => new WebEncrypter(key32, "AES-128-CBC")).toThrow(
			"The key length must be 16 bytes for AES-128-CBC.",
		);
	});
	...
	it("securely encrypts and decrypts string without serialization", async () => {
		...
		expect(Buffer.from(encrypted, "base64").toString("base64")).toEqual(
			encrypted,
		);
	...
	it("fails to decrypt if the JSON payload is unparseable", async () => {
		...
		await expect(encrypter.decrypt(invalidBase64)).rejects.toThrow(
			DecryptException,
		);
		await expect(encrypter.decrypt(invalidBase64)).rejects.toThrow(
	...
	it("fails to decrypt if payload is missing key constraints", async () => {
		...
		const incompletePayload = Buffer.from(
			JSON.stringify({ iv: "test" }),
		).toString("base64");

		await expect(encrypter.decrypt(incompletePayload)).rejects.toThrow(
			DecryptException,
		);
	});
	...
	it("fails to decrypt if the MAC is modified explicitly checking tampering", async () => {
		...
		await expect(encrypter.decryptString(tamperedPayload)).rejects.toThrow(
			DecryptException,
		);
		await expect(encrypter.decryptString(tamperedPayload)).rejects.toThrow(
			"The MAC is invalid.",
