...
function createMockSession(): SessionInterface & {
	...
	const provider: UserProvider = {
		async retrieveByCredentials(
			...
			for (const user of users.values()) {
				const userEmail = (user as unknown as Record<string, unknown>).email as
					| string
					| undefined;
				if (userEmail === email) {
					return user;
...
describe("Remember Me (Integration)", () => {
	...
	it("should retrieve user by remember token", async () => {
		...
		// Retrieve by token
		const retrieved = await provider.retrieveByToken(
