...
describe("TokenGuard — Bearer Token Auth", () => {
	...
	it("should authenticate with valid Bearer token", async () => {
		...
		expect(authenticatedUser?.getAuthIdentifier()).toBe(
	...
	it("should handle multiple requests with different tokens", async () => {
		...
		expect((await guard1.user())?.getAuthIdentifier()).toBe(
			user1.getAuthIdentifier(),
		);
		expect((await guard2.user())?.getAuthIdentifier()).toBe(
			user2.getAuthIdentifier(),
		);
	});
});
