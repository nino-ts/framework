...
const createStubContainer = (): ContainerInterface => {
	const factories = new Map<
		string,
		(container: ContainerInterface) => unknown
	>();
	...
	const container: ContainerInterface = {
		bind<T>(
			...
			factories.set(
				abstract,
				factory as (container: ContainerInterface) => unknown,
			);
		},
		bindIf<T>(
			abstract: string,
...
describe("Application", () => {
	describe("constructor", () => {
		...
		test("should cover all fallback container error methods", () => {
			...
			expect(() => app.bindIf("a", () => "x")).toThrow(
				"Container not configured",
			);
			expect(() => app.singletonIf("b", () => "y")).toThrow(
				"Container not configured",
			);
			expect(() => app.instance("c", "value")).toThrow(
				"Container not configured",
			);
